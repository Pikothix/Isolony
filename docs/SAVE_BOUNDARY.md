...
- Colonist activity, player Move command/manual destination, construction/material-delivery/harvest/haul assignment, carried payload, work/capacity reservation, movement target, transient cell path/path index, pause timer, environmental status flags, and overhead need labels.
...
- `deltas.construction_sites`: authoritative `WorldState` construction records, including building id, origin/occupied cells, required, delivered, and consumed resources, progress, build time, completion flag, and additive Storehouse `storage_contents` when present.
...
Storehouse storage component records are not serialized as a separate top-level section in version `2`. `WorldState` rebuilds components from completed Storehouse construction records whenever construction state is imported or a Storehouse completes. Storehouse contents are saved additively on the completed construction record as `storage_contents`; transient component capacity and construction-material reservations are excluded. Once any component exists, hauling, worker construction, and eating use Storehouse state exclusively; legacy-zone hauling and legacy eating remain available only with no storage component. Imported legacy totals and zones are still preserved for version-2 compatibility, and aggregate resource totals/UI counters include both compatibility totals and component contents without transferring or duplicating resources.

Delivered construction resources are authoritative and persist additively on their construction site. Construction-delivery item/worker reservations, pickup state, carrying, destinations, and paths are transient. A save taken while carrying exports the payload as a ground item at its pickup cell, matching ordinary Haul loss prevention; load clears delivery reservations and workers rediscover the work.

Construction worker/material reservations, including a no-storage legacy bootstrap earmark, harvest/haul workers, haul storage-capacity reservations, carried-item state, transient player Move commands/destinations, job candidates, and colonist cell paths/path indices are deliberately excluded from version `2` saves. Harvest creates no capacity reservation; Haul creates one transient reservation per claimed item. WorldState import clears construction allocations and legacy earmarks and restores orders/items without workers, while `Colonist.import_state()` resets player commands, work/movement targets, and path state to idle. Restored colonists rediscover work and recompute reachability from the loaded world. None of these paths refund resources already consumed into saved construction progress.
